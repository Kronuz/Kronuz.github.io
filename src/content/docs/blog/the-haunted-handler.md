...
Next familiar has a calmer job but a sharper edge. Every request that reaches Xapiand arrives as a URL, and something has to decide, fast, which of a thousand possible handlers owns it. The obvious data structure for that quietly degrades into a linear scan on exactly the inputs an attacker would send. The next one refuses to.
