...
The first thing I built wasn't inside `et` at all. It was a Python wrapper called [`etch.py`](snippet:etch.py) that ran Eternal Terminal inside a pseudo-terminal, kept the session warm, and scraped the rendered screen to hand a script back clean output and a real exit code where before there was a wall of color codes and tea leaves. It worked. It's still on my machine.
