...
Building that handle meant teaching a terminal that had only ever answered to a human to answer to something that wasn't one, and the machine on the far end did not give that up without a fight.

## First, from the outside

I did not start inside `et`. I started with a Python wrapper called [`etch.py`](snippet:etch.py), which ran Eternal Terminal inside a pseudo-terminal, kept the session warm, and scraped the rendered screen to hand a script back clean output and a real exit code where before there was a wall of color codes and tea leaves. It worked. It is still on my machine.
...
The different idea was to stop guessing. `et` has that clean stream on the inside; I just could not reach it from out here. So the version worth talking about is the one I built into the terminal itself. That's the next part.
