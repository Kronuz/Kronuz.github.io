	...
	integrations: [
		starlight({
			...
			expressiveCode: {
				...
				// Round the frame AND the editor tab to match the snippet corners.
				// Setting the core `borderRadius` (not just the frame's --ec-brdRad
				// in CSS) also feeds frames.editorTabBorderRadius, so a
				// ```text title="..." tab gets a rounded top-left corner too.
				styleOverrides: { borderRadius: '0.45rem' },
			},
			social: [
